...
class FolderMasterTest(unittest.TestCase):
    def setUp(self):
        ...
        if len(os.listdir(self.unitTestDirectory)) > 0:
            for name in os.listdir(self.unitTestDirectory):
                if os.path.isfile(self.unitTestDirectory + "\\" + name) == True:
                    os.remove(self.unitTestDirectory + "\\" + name)
                else:
    ...
    def ut_getActualOutput(self):
        for item in os.listdir(self.unitTestDirectory):
            if os.path.isfile(self.unitTestDirectory + "\\" + item) == True:
                continue
    ...
    def ut_openTestCasesFile(self, testCasesFile):
        ...
        fileContents.close()
    def utSet_performTestcase(self, testCasesFile, willSupressMainWindow = True, willSupressCaseStyleWindow = True, willSupressFormatting = True):
    ...
    def test_basic3_createMultiLevelFolder(self):
        ...
        self.ut_removeFolders()
        self.assertEqual(self.actualOutput, [])
    def test_basic4a_conflictingCasesMinimumLowercase(self):
        self.input = ["a", "A"] 
        self.chosenCases = {"a": "a"}
        self.expectedOutput = [["a"]]
        self.ut_appendFolders(self.input)
        #perform the first 2 steps from createFolders
        self.suppressNonTestOutput(
        ...
        self.ut_removeFolders()
        self.assertEqual(self.actualOutput, [])
    def test_basic4b_conflictingCasesMinimumUppercase(self):
        self.input = ["a", "A"] 
        self.chosenCases = {"a": "A"}
        self.expectedOutput = [["A"]]
        self.ut_appendFolders(self.input)
        ...
        self.ut_removeFolders()
        self.assertEqual(self.actualOutput, [])
    def test_basic4c_conflictingCases(self):
        self.input = ["AA", "aa", "Aa"] 
        ...
        self.ut_removeFolders()
        self.assertEqual(self.actualOutput, [])
    def test_basic4d_conflictingCasesCumulativeDuplicates(self):
        self.input = ["AA", "aa", "Aa"] 
        self.chosenCases = {"aa": "Aa"}
        self.expectedOutput = [["Aa"], ["Aa (Copy 1)"], ["Aa (Copy 2)"]]
        #tick the checkbox
        QtTest.QTest.mouseClick(self.form.ui.duplicateFoldersCheckbox, QtCore.Qt.LeftButton)
        self.ut_appendFolders(self.input)
        #perform the first 2 steps from createFolders
    ...
    def test_basic5c_switchPreexistingFolderCase(self):
        ...
        self.ut_removeFolders()
        self.assertEqual(self.actualOutput, [])
    def test_basic6_extensionlessFileInDirectory(self):
        with open(self.unitTestDirectory + "\\" + "Aa", "w") as extensionlessFile: 
            pass
        extensionlessFile.close()
        self.input = ["Aa"]
        self.expectedOutput = []
        #create the folders
        self.suppressNonTestOutput(
    ...
    #2a: prefixed and suffixed slashes, along with slashes just by themselves, are ignored
    ...
    def test_testcase2a_yesDupe(self):
        testCasesFile = "tests/z_testcases2a.txt"
        self.expectedOutput = [["a"], ["a (Copy 1)"], ["a (Copy 10)"], ["a (Copy 11)"], ["a (Copy 2)"], ["a (Copy 3)"], ["a (Copy 4)"], ["a (Copy 5)"], ["a (Copy 6)"], ["a (Copy 7)"], ["a (Copy 8)"], ["a (Copy 9)"]]
        QtTest.QTest.mouseClick(self.form.ui.duplicateFoldersCheckbox, QtCore.Qt.LeftButton)
        self.utSet_performTestcase(testCasesFile)
    ...
    def test_testcase2b_yesDupe(self):
        testCasesFile = "tests/z_testcases2b.txt"
        self.expectedOutput = [["a", "a"], ["a (Copy 1)", "a"], ["a (Copy 2)", "a"], ["a (Copy 3)", "a"], ["a (Copy 4)", "a"], ["a (Copy 5)", "a"]]
        QtTest.QTest.mouseClick(self.form.ui.duplicateFoldersCheckbox, QtCore.Qt.LeftButton)
        self.utSet_performTestcase(testCasesFile)
